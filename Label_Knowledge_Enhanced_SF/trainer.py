...
class Trainer:
    def __init__(self, args):
        ...
        self.train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=self.args.batch_size,
                                           num_workers=self.args.num_workers, collate_fn=collate_fn)
        self.valid_dataloader = DataLoader(valid_dataset, shuffle=False, batch_size=4,
                                           num_workers=self.args.num_workers, collate_fn=collate_fn)
        self.test_dataloader = DataLoader(test_dataset, shuffle=False, batch_size=4,
                                           num_workers=self.args.num_workers, collate_fn=collate_fn)
        # self.train_datas = []
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc=f'Train Epoch {epoch + 1}/{self.args.num_epochs}')):
                self.model.train()
                self.optimizer.zero_grad()
                
                label_knowleges_input_ids = batch['label_knowledges_input_ids'].to(self.device)  # [num_labels, L2]
                label_knowleges_attention_mask = batch['label_knowledges_attention_mask'].to(self.device)  # [num_labels, L2]
                label_knowleges_token_type_ids = batch['label_knowledges_token_type_ids'].to(self.device)  # [num_labels, L2]
    ...
    def evaluate(self, mode):
        ...
        if mode == 'test':
            valid_data = self.test_dataloader
        ...
        for batch in tqdm(dataloader, desc='Evaluating'):
            num_labels = batch['num_labels']  # int
            input_ids = batch['input_ids'].to(self.device)  # [B, L1]
            attention_mask = batch['attention_mask'].to(self.device)  # [B, L1]
            token_type_ids = batch['token_type_ids'].to(self.device)  # [B, L1]
